    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
{
    ...
    try
    {
        ...
        var aiAnalysisConsumer = new AsyncEventingBasicConsumer(channel);
        aiAnalysisConsumer.ReceivedAsync += async (model, ea) =>
        {
            await ProcessAiAnalysisMessageAsync(ea, stoppingToken);
        };
        ...
        _logger.LogInformation(
            "Подключено к очередям RabbitMQ: pdf_report_queue, excel_report_queue, ai_analysis_queue");
    ...
    private async Task ProcessAiAnalysisMessageAsync(
        BasicDeliverEventArgs ea,
        CancellationToken cancellationToken)
    {
        var channel = GetRabbitMqChannel();
        ...
        try
        {
            ...
            var aiTask = JsonSerializer.Deserialize<AiAnalysisTask>(
                message,
            ...
            if (aiTask == null)
            {
                _logger.LogError("Не удалось десериализовать задачу AI-анализа");
            ...
            await coordinator.GenerateAiAnalysisAsync(aiTask);
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Ошибка обработки AI-анализа {AnalysisId}",
                analysisId);
